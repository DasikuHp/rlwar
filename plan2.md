...
| Sesión 6 (2026-09-24) | **Auditoría de las sesiones 4 y 5** (tabla en el relevo s6). **Duelo mix**: cada partida se absorbía dos veces (recuerdos repetidos, rival con 12 partidas en vez de 6, cada término contado dos veces); decisión: "una vez, el repaso reutiliza" las mismas partidas con sus recompensas, como el entreno (spec/06 §6.2). **Aprender en un hilo, también las exhibiciones** ("sí, también"): la red queda ocupada mientras aprende; y, por lo que dice spec/11, a cualquier velocidad (x1/x10) y la muestra del paso de evolución. **Quedarse + afinar** se queda como dice la spec (pide un reajuste pequeño; si es imposible, castigo). **Carpetas gw-* de TEMP**: se borran las de ahora y run-all borra al acabar las que crea la batería. **La media de la ventaja empieza en 0** ("primera ventaja = G − 0") y Adam empieza en t = 0, escrito en spec/04. Premisas medidas: el 0,85 s de la sesión 5 no se reproduce (0,15–0,5 s con lote 4); `hilos-c` usa redes con la capa de Instinto a 256 neuronas y lotes de 8. |
| Sesión 7 (2026-09-24) | **Prueba visual de P5**: "estoy muy feliz con la UI; se puede pulir un poco mejor pero está muy muy bien". **Tutorial (P10)**: "que te enseñe a hacer una red desde 0, qué afecta a qué, y cómo añadir cosas de las 3 complejidades y de cada fase": el recorrido construye una red desde Vacía explicando qué cambia cada bloque en el comportamiento, enseña los bloques de Aprendiz, Artesano y Científico, y pasa por las 4 etapas. **Ficha de red**: "encima, con lengüeta": sale por la derecha encima de la etapa al pulsar el nombre de cualquier red y, retraída, queda una lengüeta con su emblema. **Etapa 4**: Trono y duelos · Dinastías · **Crónica** (Verdad y Quirófano solo en la ficha). |
| Sesión 8 (2026-09-25) | **Prueba de P5** (sin probar aún los entrenos): (1) "bajar el webfluid" → ajuste nuevo **Tinta de la portada** Suave (por defecto) · Intensa · Apagada; (2) "poder eliminar las partidas guardadas" → **Cargar o borrar**, confirmación con un segundo clic que dice qué se pierde (el servidor sigue pidiendo el nombre y lo manda el cliente) y, con las 3 ranuras llenas, Nueva partida abre ese diálogo; (3) "poder crear redes desde 0; quiero que el tutorial sea eso" → plantilla **Desde cero** (red sin bloques) y el tutorial (P10) empezará ahí, no en Vacía; (4) "la comunicación en la partida tiene que ser mejor, mira el original; los bocadillos, bastante malos" → como el original: **barra de la función** bajo el plano (quién tira, "y = …" / "y' = …" / "y'' = … · ángulo" y lo que consigue cuando cae la curva), **nombres en etiqueta** sobre cada soldado, **registro con los nombres en negrita y del color de su bando** (el resultado de un tiro no sale antes de que caiga su curva) y **bocadillos** en cola (primero la función, luego lo que dice; uno por soldado; sin pisarse, con línea guía); (5) "los ajustes no cambian todo" → se aplican al momento en todo el juego (calidad y movimiento también en el plano del duelo; la velocidad es la marcada en duelos y retos al trono, turbo incluido); (6) "en x10 quiero ver las líneas siempre" → la curva se traza a la velocidad de la sala y todas las de la partida se quedan, cada vez más tenues. **Título**: en dos líneas a 1280 está bien. **Editor (P6)**: las tarjetas se quedan ("a día de hoy está muy bien"), pero **más explicadas y explícitas**, y si algo no se puede, **una pista de dónde podría ir o de qué le falta a tu red** para poder. **Columnas**: por el orden de la señal, con los títulos del mockup según lo que haya ("el mockup es una referencia visual, cúrratelo"). |
| Sesión 9 (2026-09-25) | **Auditoría de la s8** (relevo s9). El usuario no probó la lista de la s8, pero miró el tutorial (lo que hay: "Tu red está vacía" de Desde cero y las pistas): **"sigue sin ser didáctico y explicativo; busca bien cómo se hace un buen tutorial"** → antes de rehacer las pistas y de diseñar P10, investigar con fuentes cómo se hace un buen tutorial y preguntarle el diseño. **Versiones**: "las de entreno nunca se borran" → las de antes del entreno se guardan siempre; el tope de 50 poda solo las del editor y las de volver a una versión (spec/04 §11.7). **Probar ya y el duelo** comparten una sola escucha de sala (`game/sala.js`, adelantado de P7 porque pintaban las dos salas en el mismo plano). **Encajar** = que la red quepa entera, a lo ancho y a lo alto (55–100 %). **Moviola en el mismo sitio**: la partida guardada no trae curvas ni posiciones iniciales; propuesta en el relevo s9 (añadir `positions` a `game.start` y rehacer las curvas con el solver). |
| Sesión 10 (2026-09-25) | El usuario no probó nada nuevo; `referencianoabsoluta.png` → **a git**; `test/huecos-s4.spec.mjs` → **a git, fuera de la batería hasta P11**. Luego: **"no preguntes, voy a dormir"** → lo de diseño de esta fila lo decidí yo con la opción más completa y es **revisable** (dilo y se cambia). **Investigación del tutorial** (fuentes): Bret Victor, *Learnable Programming* (worrydream.com/LearnableProgramming): "show the data", "create by reacting", "start constant, then vary"; Nicky Case, *How I Make Explorable Explanations* (blog.ncase.me/how-i-make-an-explorable-explanation): empezar por una pregunta ("make them love your question"), subir la escalera de lo concreto a lo abstracto y acabar en caja de arena; Case, *4 More Design Patterns* (blog.ncase.me/explorable-explanations-4-more-design-patterns): "Place your bets" (apostar antes de ver) y "Sandbox mode"; Andersen et al., CHI 2012, *The impact of tutorials on games of varying complexity* (dl.acm.org/doi/10.1145/2207676.2207687, 45 000 jugadores): el tutorial solo compensa en juegos complejos y rinde más pegado a lo que enseña; Human Resource Machine (tomorrowcorporation.com/humanresourcemachine): empieza con 2 órdenes y suma una cada vez; Miyamoto sobre el 1-1 de Super Mario Bros. (gamedeveloper.com/design/how-miyamoto-built-i-super-mario-bros-i-legendary-world-1-1): enseñar haciendo, sin texto; Hayashida / GMTK, *Super Mario 3D World's 4 Step Level Design* (youtube.com/watch?v=dBmIkEvEBtA): introducir, desarrollar, **giro**, cerrar; Renkl y Atkinson 2004, *How Fading Worked Solution Steps Works* (Instructional Science, link.springer.com/article/10.1023/B:TRUC.0000021815.74806.f6): ayuda completa al principio que se retira; NERO (Stanley, nn.cs.utexas.edu/nero/about.php): entrenar con escenas de menos a más; TensorFlow Playground (Smilkov, Carter et al., arxiv.org/abs/1708.03788): tocar y ver al momento, sin código. **Diseño (decidido sin preguntar)**: guía **«Tu primera red»** de 9 pasos en una tarjeta sobre el lienzo, cada paso = pregunta → una sola cosa que hacer (iluminada) → lo que ha cambiado, con los números del banco: 1 Candidatos · 2 Elegir · 3 cable · 4 **apuesta** "¿sabe apuntar?" (certeza real ≈ 0 → sin entrenar escoge casi al azar) · 5 Instinto **en medio del cable** · 6 **giro**: la roca · 7 Destinos → Moverse · 8 Rasgos → Eco → Instinto · 9 guardar y Probar ya → caja de arena. «Hazlo por mí» en los pasos 1–3; después, «No lo encuentro» (ilumina y ofrece hacerlo). Sale al crear Desde cero y con el botón **Guía**; el avance va a `meta.tutorial.primeraRed` del mundo (lo usará P10). Opciones que no elegí: tutorial a pantalla completa aparte del editor; vídeo o texto largo antes de empezar; retos con examen antes de pasar ("Puzzle it out", queda para P10). **Nivel del editor = el del mundo** (A/B/C). **+ Añadir capa** sobre un cable: ＋ en la mitad del cable elegido y la lista de lo que cabe. |
| Sesión 11 (2026-09-25) | **Auditoría de la s10 con ratón de verdad** (la s10 solo probó con `.click()` de JS): la paleta se movía bajo el ratón (su explicación crecía y empujaba la lista; el clic caía en otro bloque) → la guía no pasaba del paso 1; a 1280 × 800 el «👉» y «Hazlo por mí» quedaban cortados; la escena del banco asomaba 95 px. Arreglado en b219bda. **El usuario probó la guía y se atascó**: quiere un botón de **saltarlo todo**; que el tutorial **guíe mucho más y se enfoque solo en lo que hay que hacer** ("hoy entras, ves todo el pifostio y luego el tutorial"); poder **hacer tu primera red**; que al acabar **entiendas cada cosa que puedes y debes hacer** ("no es borrar cosas: que se explique solo"); y **enchufar y desenchufar bloques fácil** (sondeo: arrastrar desde la entrada mueve la tarjeta, el punto de salida mide 7,5 px a 58 %, quitar un cable exige acertar a la línea y buscar «Quitar cable» abajo). **Prioridad: el tutorial** (antes que la moviola). Investigación con fuentes y propuesta en **spec/12-tutorial.md**: modo taller (el editor se construye pieza a pieza, solo se ve lo que se usa), 8 capítulos y graduación, reto sin ayuda al final de cada uno, mano fantasma, saltar todo o cada capítulo, "nada sin presentar" con test; enchufar/desenchufar como Blender y Unreal. **Sin aprobar**: sus preguntas (§6) van al principio de la sesión 12. |
| Sesión 12 (2026-09-25) | **Respuestas del usuario a spec/12 §6.** (1) **Dónde**: el **editor oficial, en penumbra, que «se va encendiendo»** (idea del usuario, con su maqueta): todo empieza a oscuras; lo que toca se ilumina con un halo azul y una ventana del Sistema unida por una línea con un punto; lo ya aprendido queda a media luz, lo no visto a oscuras; al acabar, todo encendido. Solo responde lo iluminado (más «Saltar tutorial»); se retoma con **Guía**. Sustituye al «modo taller» que construía el editor pieza a pieza. Maqueta: ventana «1/4 Elige una plantilla» con título, texto, puntos de avance y botón redondo → para los pasos de presentar. (2) **Duración**: completo, 8 capítulos + graduación (≈25 min), cada uno saltable, pausa para Probar ya tras el 4. (3) **Voz**: neutra, **«el Sistema»**, en **ventana del Sistema** (azul translúcido, borde que brilla, título «SISTEMA», texto que entra línea a línea). (4) **Mano fantasma**: en cada gesto nuevo, y después solo si te atascas. (5) **Retos** sin ayuda al final de cada capítulo, con «Hazlo por mí» tras 2 fallos, saltables. (6) **Soltar una tarjeta encima de un cable la mete en medio**, con aviso antes de soltar (el cable se ilumina y se abre un hueco); Ctrl+Z lo deshace. **Hecho (795a196)**: enchufar y desenchufar (spec/12 §5), con lógica pura y test (`plug.js`, `test/ui-enchufe.spec.mjs`) y un sondeo con ratón real a 1280 y a 1920; el sondeo destapó tres fallos, ya arreglados (tarjeta que iba por detrás del ratón, cables que la esquivaban, `patch()` que rompía los `<path>`). **El tutorial, solo analizado**: su arquitectura está en `spec/relevo-2026-09-25-s12.md` §4. La sesión se cerró al pasar ~300k de contexto. |
| Sesión 13 (2026-09-25) | **Tutorial hecho** (spec/12 §4 con lo aprobado en la s12; arquitectura del relevo s12 §4). Motor puro `public/js/game/tutorial/engine.js` (avanzar, saltar, luces, escalón de ayuda, retos con su cambio; el tiempo entra como `now`), capítulos como datos `crear.js` (8 y la graduación, 56 pasos; reutiliza preguntas y «lo que ha cambiado» de `coach.js`; `ensure` por capítulo para «Ya sé esto» y «Ya sé algo»), capa `overlay.js` (penumbra con máscara SVG: iluminado / media luz / a oscuras; solo responde lo iluminado; ventana del Sistema como la maqueta, pegada a lo iluminado sin taparlo; barra de capítulos con «Ya sé esto» y «Saltar tutorial»; mano fantasma). Efectos nuevos en `anim.js` (texto línea a línea, mano, chispas, cable que se dibuja, números que cuentan), con «reducir movimiento». La guía de la s10 (tarjeta en la esquina) se retira; el botón **Guía** retoma el tutorial. Avance en `meta.tutorial.crear` del mundo; un mundo nuevo lo empieza solo. Pruebas: `test/ui-tutorial.spec.mjs` (13 casos: motor, `ensure`, recorrido entero con «Hazlo por mí», **nada sin presentar**) y `tools/sondeo-tutorial.mjs` con ratón real (276 comprobaciones a 1280, 278 a 1920, también con «reducir movimiento»). Decisiones menores tomadas sin preguntar (se pueden cambiar): la barra del tutorial va sobre la de etapas; los retos borran el deshacer (si no, Ctrl+Z los resolvía); en la graduación cada zona se ilumina 1,5 s. |
| Sesión 14 (2026-09-25) | **Moviola en el mismo sitio: P6 cerrada.** Al acabar «Probar ya», «Ver otra vez» con las 6 partidas (ganó/perdió/empate); cada una se ve en el mismo plano con ▶, pausa (también a mitad de curva), ⏭ tiro a tiro, ⏮ atrás, deslizador para ir a cualquier tiro y velocidades x½, x1, x3, x10; la barra dice quién tira, qué función, qué pasó y adónde se movió; el marcador, qué partida, qué mapa y de qué color es tu red. Servidor: `game.start` lleva `positions` (dónde empieza cada soldado); `shared/moviola.js` rehace cada tiro con el solver. **Hallazgo**: el `Math.sin`/`exp` de Chrome y el de Node difieren en el último bit, así que en el navegador la curva rehecha no es exacta (9–14 de 90 no cuadraban); por eso las rehace el servidor (`GET /api/lab/games/:id?solo=jugadas`, que además baja la partida de ~9 MB a ~400 kB) y el navegador solo dibuja. El panel de Probar ya ocupa también la franja de abajo mientras está abierto (a 1280 el plano tenía 120 px). Test `ui-moviola.spec` (curvas contra el SSE, punto por punto) y sondeos con ratón real `sondeo-moviola` (la pausa del capítulo 4 con un duelo entero) y `sondeo-retos` (**«Hazlo por mí» de 3 retos no hacía nada** si faltaba lo construido antes en el capítulo —recargar sin guardar, borrar bloques—: ahora pone también eso). |

...
| P0-B3 | bajo | `training.js:61` | "En la de evolución, todos": solo es verdad si `population × gamesPerCandidate ≥ hilos` (`evo/train.js:650`). | `threadNote` con `evolution.population` 2 y 8 hilos. | ✅ `ui-arreglos-p0` |
| P0-B4 | bajo | `public/js/lab/whatif.js:28-31` (`compare`) | Compara la elección por el índice del candidato: si la red editada cambia su Imaginación, el #i es otro tiro y "misma elección" puede mentir. | Cambiar las familias de la Imaginación en el editor y mirar "¿qué pasaría si…?". | ✅ spec/08 §12, `ui-arreglos-p0` |
| P0-B5 | bajo | `training.js:40` (`trainingBody`) | Con la dureza vacía el formulario manda 0; el servidor usa 2 por defecto (`evo/league.js:82`). | Vaciar "Dureza" (Científico) y entrenar. Lo mismo con los fantasmas y la ganancia mínima de la meseta (0,02 en el servidor). | ✅ spec/08 §12, `ui-arreglos-p0` |
...
| P0-B9 | bajo | `evo/train.js` (`record`) | Las partidas de muestra de un entreno se guardaban con `soldiers` vacío: `record()` recibía el resultado de la partida, que no trae los soldados. | `GET /api/lab/games?trainingId=…` → `soldiers` sin valor. | ✅ con la receta (`receta-entreno`, currículo) |
| P0-D1 | diseño | `evo/mutate.js`, crías de la API y de las dinastías | Dos crías de la misma madre repetían nombres ("Lince Veloz-2a" dos veces; en dinastías, "hydra-1a" dos veces). | `nombres-p0`. | ✅ decisión del usuario: la primera letra libre (spec/05 §10); `api-evolucion` cambiado con su OK |
| P0-D2 | diseño | spec/04 §9.4 | `createTrainer` prometía un `learnCfg` que no existía. | — | ✅ decisión del usuario: receta de entreno completa (spec/04 §11) |
